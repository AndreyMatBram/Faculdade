...
void SelectionSort(Dados L[MAX],int Aux){
    int Cont1, Cont2, Min;
        ...
        if (L[Min].Valor < L[Aux].Valor){
            Min = Aux;                
        }
        
        SelectionSort(L,Min+1);
...
void InsertionSort(Dados L[MAX]){
    ...
    for(Cont1=1;Cont1<MAX;Cont1++){
        ...
        //Basta Mudar o sinal que compara os valores para alterar de crescente para decrescente
...
void BubbleSort(Dados L[MAX]){
    ...
    do{    
        ...
        for (Cont2=MAX-1;Cont2>Cont1;Cont2--) {
			//Basta trocar o sinal deste 'if' para trocar de de   
        ...
        while (ContA <= Meio || ContB <= Fim){
            if (ContA > Meio){ //final do VetorA
                ...
                if (ContB > Fim){ //final do VetorB
                	...
                	//basta mudar o sinal deste 'if' para trocar de crescente para decrecente
...
void QuickSort(Dados L[MAX], int Inicio, int Fim){
    ...
    if (Inicio < Fim){
        ...
        for(Cont=Inicio+1;Cont<=Fim;Cont++){        
            ContAux = Cont;  
			//Esse 'if' determina se é crescente ou decrecente                    
...
void QuickSort2(Dados L[MAX], int Inicio, int Fim){
    int Pivo;
    ...
    if (Inicio < Fim){
        ...
        QuickSort2(L, Inicio, Pivo-1);
        QuickSort2(L, Pivo+1, Fim);
    }
}
    ...
    if (Inicio < Fim){
        ...
        do{    
            ...
            if (ContFim > Pivo){
                Swap(L, Pivo, ContFim);
                if (Msg){
                    cout << endl << "Troca: " << Pivo << " por " << ContFim;    
                }
            ...
            if (ContIni < Pivo){
                Swap(L, ContIni, Pivo);
                if (Msg){
                    cout << endl << "Troca: " << ContIni << " por " << Pivo;
                }    
                Pivo = ContIni;
            }    
